...
def send(sbj, text, to):
    ...
    account = acc
    password = pw
    reply_to = "bwc.RGLITTeam@nhs.net"
    server.login(account, password)
    msg = email.mime.multipart.MIMEMultipart()
    msg["From"] = "ORGLStock@gmail.com"
    msg["Subject"] = sbj
    msg.add_header("reply-to", reply_to)
    text += "You can visit the site at: <a href='{0}'> {0} </a><br><br>".format(
        settings.SITE_URL
    )
    text += "This message was automatically sent on behalf of the Stock Team.<br><br>"
    html = '<html><head></head><body>{}If you have any issues please contact <a href="mailto:bwc.RGLITTeam@nhs.net?subject=Stock%20Database">bwc.RGLITTeam@nhs.net</a> or reply to this message.</p></body></html>'.format(
        text
    )
